...
<ParamField path="points" type="Vec2[] | {x: number, y: number}[]" required>
  The ordered list of vertices that define the path. At least two points are required.
</ParamField>
...
Appends a single vertex. `u` and `v` are UV coordinates (default `0`). `color` is a packed `uint32` — use `rapid.getColorUint32(myColor)` or pass `0xFFFFFFFF` for white.
