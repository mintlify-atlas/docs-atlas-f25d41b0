...
description: "API reference for CustomGlShader: inject vertex and fragment GLSL hooks into the Rapid.js sprite and graphic pipelines, set uniforms, and chain filter passes."
---
...
const hueShift = new CustomGlShader(
  ...
  void fragment(inout vec4 color) {
    ...
    float len     = dot(color.rgb, weights);
    color.rgb = vec3(
      len + (color.r - len) * c - color.g * weights.g * s,
      len + (color.g - len) * c + color.r * weights.r * s,
      len + (color.b - len) * c
    );
  }
  `,
